...
(function () {
  ...
  try { document.documentElement.setAttribute("data-notte-build", "v2.3-token-variants"); } catch (e) {}
  ...
  function buildOverride(root) {
    ...
    // Pass 1: learn which custom properties are colors, so usages can be
    // rewritten to role-specific variants.
    var colorVars = new Set();
    for (var i = 0; i < col.readable.length; i++) {
      try { collectColorVarNames(col.readable[i].cssRules, colorVars); } catch (e) {}
    }
    if (root === document) lastColorVars = colorVars;
    // Pass 2: emit the override CSS.
    var ctx = { out: [], cors: [], colorVars: colorVars };
