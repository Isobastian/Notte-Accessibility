...
export function remap(rgb, kind, theme) {
  ...
  if (kind === "bg") {
    ...
    // Text: keep hue, then raise lightness until it meets WCAG AA (4.5:1)
    // against the lightest surface in our band — so red-on-dark, grey-on-dark,
    // etc. are all guaranteed readable, not just "inverted".
