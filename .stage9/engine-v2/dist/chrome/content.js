(() => {
  ...
  function collectColorVarNames(rules, set) {
    if (!rules) return;
    for (var i = 0; i < rules.length; i++) {
      try {
        ...
        if (rule.style && rule.selectorText !== void 0) {
          ...
          try {
            if (rule.styleSheet.cssRules) collectColorVarNames(rule.styleSheet.cssRules, set);
          } catch (e) {
          }
  ...
  (function() {
    ...
    try {
      document.documentElement.setAttribute("data-notte-build", "v2.3-token-variants");
    } catch (e) {
    }
    ...
    function buildOverride(root) {
      ...
      var colorVars = /* @__PURE__ */ new Set();
      for (var i = 0; i < col.readable.length; i++) {
        try {
          collectColorVarNames(col.readable[i].cssRules, colorVars);
        } catch (e) {
        }
      }
      if (root === document) lastColorVars = colorVars;
      var ctx = { out: [], cors: [], colorVars };
