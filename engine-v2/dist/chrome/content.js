(() => {
  ...
  (function() {
    ...
    try {
      document.documentElement.setAttribute("data-notte-build", "v2.23-before-paint");
    } catch (e) {
    }
    ...
    var COVER_QUIET = 1e3;
    var COVER_MIN_HOLD = 1e3;
    var COVER_HARD_CAP = 6e3;
    ...
    function startCoverObserver() {
      if (coverObserver || !loadingCover) return;
      try {
        coverObserver = new MutationObserver(function(muts) {
          if (!loadingCover) return;
          for (var i = 0; i < muts.length; i++) {
            var added = muts[i].addedNodes;
            for (var j = 0; j < added.length; j++) {
              var n = added[j];
              if (n.nodeType === 1 && !(n.getAttribute && n.getAttribute("data-notte") !== null)) {
                noteCoverActivity();
                return;
              }
            }
