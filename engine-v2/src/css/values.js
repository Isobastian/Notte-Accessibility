 ...
 * For var(...), the variable *reference* is kept intact (its own *definition* is
 * transformed elsewhere), but we DO remap color literals in the fallback — the
 * part after the first top-level comma, e.g. var(--x,#fff). That fallback is the
 * value the browser paints when the variable is unset, which is exactly how
 * LightningCSS/Tailwind light-dark() surfaces work, so it must be darkened.
 *
...
// Index of the first comma at paren-depth 0 (splits a var()'s name from its
// fallback), or -1 if there is no top-level comma.
  ...
  while (i < n) {
    ...
    if (IDENT_START.test(ch)) {
      ...
      if (value[k] === "(") {
        ...
        if (lname === "url") {
          out += whole; i = close + 1; continue;         // keep images verbatim
        }
        if (lname === "var") {
          // Keep the variable name; remap color literals in the fallback (after
          // the first top-level comma). Fixes LightningCSS/Tailwind light-dark().
          var vinner = value.slice(k + 1, close);
          var cpos = topLevelComma(vinner);
          if (cpos === -1) {
            out += whole;                                 // var(--x) — no fallback
          } else {
