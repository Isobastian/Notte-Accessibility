 ...
 */
import { walkRules, collectVarDefs, resolveColorVars } from "./css/rules.js";
...
(function () {
  ...
  try { document.documentElement.setAttribute("data-notte-build", "v2.9-cover-fix"); } catch (e) {}
  ...
  // Hard backstop: no matter what happens during theming (an error, a hung
  // fetch, weird load timing), the cover MUST come off — otherwise the page is
  // stuck flat #141414/#e8e6e3 with no real colors. Independent of applyTheme.
  setTimeout(liftCover, 1500);
  ...
  function collectVarDefsFrom(root, map) {
    ...
    for (var i = 0; i < col.readable.length; i++) {
      try { collectVarDefs(col.readable[i].cssRules, map); } catch (e) {}
    }
  }
  ...
  function process() {
    ...
    // Global token pass across document + all shadow roots (incl. chains), then emit.
    var varMap = {};
    collectVarDefsFrom(document, varMap);
    for (var k = 0; k < shadowRoots.length; k++) { try { collectVarDefsFrom(shadowRoots[k], varMap); } catch (e) {} }
  ...
  function decide(s) {
    if (s.dark[host] === false) return false;  // dark-mode feature switched off for this site
