...
ok("neutral rows both stay dark", lum(g.rowA.bg) < 25 && lum(g.rowB.bg) < 45);
ok("selected row differentiated from base row", lum(g.rowB.bg) - lum(g.rowA.bg) >= 3);
...
ok("button distinct from neutral black", lum(g.btn.bg) > lum(g.body.bg));
