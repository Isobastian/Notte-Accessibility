...
export function remap(rgb, kind, theme) {
  ...
  if (kind === "bg") {
    if (neutral) {
      hsl.s = 0;
      // Narrow dark band, PROPORTIONAL to the input (not one flat black):
      // lighter neutrals map to slightly lighter darks. This keeps the subtle
      // white / hover / selected / unread greys that apps like Outlook Web and
      // Fluent use for row states distinguishable, and gives cards/panels a
      // little lift off the page, instead of collapsing every grey to the same
      // black (which read as "no hover, no differentiation"). Pure white -> 8
      // (= the page base #141414); greyer surfaces -> up to a capped ~20.
      L = Math.min(8 + (100 - hsl.l) * 0.34, 20);
...
// remapShadow: box-shadow colors. Fluent/Outlook draw panel borders, focus
// rings and elevation with box-shadow, and a LIGHT shadow reads as a bright
// stroke around a now-dark panel. So: darken bright shadow colors (remap as a
// background) to kill the stroke; leave already-dark shadows essentially
// untouched, so a normal drop shadow doesn't turn into a lightened halo.
export function remapShadow(rgb, theme) {
  var a = (rgb.a === undefined) ? 1 : rgb.a;
  if (luminance(rgb) >= 140) return remap(rgb, "bg", theme);   // bright stroke/glow -> darken away
  return "rgba(" + rgb.r + "," + rgb.g + "," + rgb.b + "," + a + ")"; // normal dark shadow -> keep
}
