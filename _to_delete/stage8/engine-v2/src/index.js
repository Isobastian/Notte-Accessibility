 ...
 */
import { walkRules } from "./css/rules.js";
...
(function () {
  ...
  try { document.documentElement.setAttribute("data-notte-build", "v2.2-cf-harmonize"); } catch (e) {}

  var theme = makeTheme("dark");
  var shadowRoots = [];
  var fetchedHrefs = Object.create(null);
  var watcher = null;
  var inline = createInlineManager(function () { return theme; });
  ...
  function buildOverride(root) {
    ...
    for (var i = 0; i < col.readable.length; i++) {
      try { walkRules(col.readable[i].cssRules, theme, ctx); } catch (e) {}
    }
