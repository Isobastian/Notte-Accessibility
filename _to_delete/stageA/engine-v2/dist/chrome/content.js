(() => {
  ...
  (function() {
    ...
    try {
      document.documentElement.setAttribute("data-notte-build", "v2.4-global-tokens");
    } catch (e) {
    }
    ...
    function collectVarsFrom(root, set) {
      ...
      for (var i = 0; i < col.readable.length; i++) {
        try {
          collectColorVarNames(col.readable[i].cssRules, set);
        } catch (e) {
        }
      }
    }
    ...
    function process() {
      ...
      for (var k = 0; k < shadowRoots.length; k++) {
        try {
          collectVarsFrom(shadowRoots[k], colorVars);
        } catch (e) {
        }
      }
