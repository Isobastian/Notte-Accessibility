...
export function removeAntiFlash(root) {
  root = root || document;
  // Find it wherever it was appended. At document_start <head> often doesn't
  // exist yet, so the cover goes onto <html>; looking only in <head> at removal
  // time then MISSES it, and the flat cover stays forever — flattening the whole
  // page to #141414/#e8e6e3 and hiding every real color (the OWA bug).
  var el = null;
