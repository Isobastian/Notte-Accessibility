...
// box-shadow is transformed via the "shadow" role (remapShadow): bright shadow
// strokes/glows are darkened, normal dark shadows kept — see remap.js.
// text-shadow / SVG fill / stroke are still left untouched (fill/stroke risk
// recoloring icon glyphs) — documented next-iteration seams.
...
export function transformDeclaration(style, theme) {
  ...
  for (var i = 0; i < style.length; i++) {
    ...
    if (!value) continue;
    // Custom-property definitions go through transformVarDef (handles literal
    // colors AND raw rgb-channel tokens); everything else through transformValue.
